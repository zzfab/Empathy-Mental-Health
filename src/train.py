...
emp_wdir = os.getcwd()
print(emp_wdir)
sys.path.append(os.path.join(emp_wdir,'subrepo/Empathy-Mental-Health/src'))
...
for i in df.level.values.astype(float):
    ldict[i] += 1
print('label dist: ' + str(ldict))
labels = df.level.values.astype(float)  # int
...
if args.do_validation:
    ...
    # df_val['level'] *= 2
    val_labels = torch.tensor(df_val.level.values.astype(float))  # int
...
if args.do_test:
    ...
    # df_test['level'] *= 2
    test_labels = torch.tensor(df_test.level.values.astype(float))  # int
...
Load model
'''
model = BiEncoderAttentionWithRationaleClassification(hidden_dropout_prob=args.dropout, empathy_num_labels=1)
...
for epoch_i in range(0, args.epochs):
    ...
    if args.do_validation:
        ...
        for batch in validation_dataloader:
            ...
            total_eval_accuracy_empathy += flat_accuracy(logits_empathy, label_empathy_ids, 3)
            total_eval_accuracy_rationale += 0  # flat_accuracy_rationale(logits_rationale, label_rationale_ids, label_empathy_ids, rationale_lens, axis_=2)

            pos_f1_empathy, micro_f1_empathy, macro_f1_empathy = compute_f1(logits_empathy, label_empathy_ids, 3)
        ...
        print("  AVG LOSS: {0:.5f}".format(avg_val_loss))

        if avg_val_accuracy_empathy > best_val:
            torch.save(model.state_dict(), args.save_model_path)
            best_val = avg_val_accuracy_empathy
